...
inline std::string getResolvedUrdf()
{
  ...
  std::string urdf_out = "/tmp/mobile_manipulator_test_deterministic.urdf";
...
class DeterministicJointSpaceTest : public ::testing::Test
{
protected:
  void SetUp() override
  {
    ...
    test_node_ = rclcpp::Node::make_shared("test_deterministic_joint_space_node", options);
...
TEST_F(DeterministicJointSpaceTest, NamedPoseUsesPilzPtpPlanner)
{
  ...
  // Assert correct planning pipeline and planner ID were set
  EXPECT_EQ(last_goal->request.pipeline_id, "pilz_industrial_motion_planner");
  EXPECT_EQ(last_goal->request.planner_id, "PTP");
}
