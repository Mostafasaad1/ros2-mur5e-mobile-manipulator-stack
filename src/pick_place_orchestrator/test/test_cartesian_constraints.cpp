...
inline std::string getResolvedUrdf()
{
  ...
  std::string urdf_out = "/tmp/mobile_manipulator_test_cartesian.urdf";
...
class MockMoveGroupActionServer : public MockActionServer<moveit_msgs::action::MoveGroup>
{
public:
  MockMoveGroupActionServer(rclcpp::Node::SharedPtr node, const std::string & name)
...
class CartesianConstraintsTest : public ::testing::Test
{
protected:
  void SetUp() override
  {
    ...
    test_node_ = rclcpp::Node::make_shared("test_cartesian_constraints_node", options);
    mock_node_ = rclcpp::Node::make_shared("mock_action_node");

    // Start mock action servers
    mock_move_group_ = std::make_unique<MockMoveGroupActionServer>(mock_node_, "move_action");
    mock_move_group_2_ = std::make_unique<MockMoveGroupActionServer>(mock_node_, "move_group");
  ...
  std::unique_ptr<MockMoveGroupActionServer> mock_move_group_;
  std::unique_ptr<MockMoveGroupActionServer> mock_move_group_2_;
...
TEST_F(CartesianConstraintsTest, TargetPoseAppliesLinAndOrientationConstraints)
{
  ...
  blackboard_->set("z_offset", 0.10);
  blackboard_->set("x_offset", 0.20);
  ...
  auto last_goal = mock_move_group_->get_last_goal();
  if (last_goal == nullptr) {
    last_goal = mock_move_group_2_->get_last_goal();
  }
  ASSERT_NE(last_goal, nullptr);

  // Verify pipeline and planner settings
  EXPECT_EQ(last_goal->request.pipeline_id, "pilz_industrial_motion_planner");
  EXPECT_EQ(last_goal->request.planner_id, "LIN");

  // Verify orientation path constraints
  const auto & constraints = last_goal->request.path_constraints;
  ASSERT_EQ(constraints.orientation_constraints.size(), 1u);

  const auto & ocm = constraints.orientation_constraints[0];
  EXPECT_EQ(ocm.link_name, "ur5e_tool0");
  EXPECT_NEAR(ocm.absolute_x_axis_tolerance, 0.02, 1e-4); // roll locked
  EXPECT_NEAR(ocm.absolute_y_axis_tolerance, 0.02, 1e-4); // pitch locked
  EXPECT_GT(ocm.absolute_z_axis_tolerance, 6.0);           // yaw unconstrained (>6.0 rad)
}
