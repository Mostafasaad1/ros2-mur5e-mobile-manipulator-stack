...
inline std::string getResolvedUrdf()
{
  ...
  std::string urdf_out = "/tmp/mobile_manipulator_test_isolation.urdf";
...
public:
  ...
  : MockActionServer(node, "optimize_placement", true) {}
...
public:
  ...
  : MockActionServer(node, name, true) {}
...
public:
  ...
  : MockActionServer(node, "execute_trajectory", true) {}
...
{
public:
  MockNavServer(rclcpp::Node::SharedPtr node)
  : MockActionServer(node, "navigate_to_pose", true) {}
};

class PickIsolationTest : public ::testing::Test
{
protected:
  void SetUp() override
  {
...
TEST_F(PickIsolationTest, SkipsPlacePhaseIfPoseIsEmpty)
{
  ...
  // Empty Place Pose
  goal.place_pose.header.frame_id = ""; // Empty frame signifies not provided

  std::vector<int> step_indices_received;

  auto send_goal_options =
    rclcpp_action::Client<pick_place_orchestrator::action::PickPlaceMission>::SendGoalOptions();
  send_goal_options.feedback_callback =
    [&step_indices_received](
    rclcpp_action::ClientGoalHandle<pick_place_orchestrator::action::PickPlaceMission>::SharedPtr,
    const std::shared_ptr<const pick_place_orchestrator::action::PickPlaceMission::Feedback>
    feedback) {
      step_indices_received.push_back(feedback->step_index);
    };
  ...
  // Verify that the step index never went past 6 (STOW_ARM_PICK)
  for (int idx : step_indices_received) {
    EXPECT_LE(idx, 6);
  }
}
